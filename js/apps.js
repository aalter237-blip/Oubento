...
APPS.distros = {
  mount(el) {
    const paint = () => {
      const cur = currentDistro();
      el.innerHTML = `
        <div class="disclaimer">${t("distroHint")}</div>
        <div class="pad"><b>${t("currentSession")}:</b> ${OS.settings.lang === "ar" ? cur.ar : cur.name} · ${cur.session}</div>
        <div class="distro-grid">
          ${DISTROS.map((d) => `
            <button class="distro-card ${d.id === cur.id ? "on" : ""}" data-id="${d.id}">
              <div class="distro-swatch" style="background:linear-gradient(145deg,${d.color},${d.accent})"></div>
              <div class="grow">
                <b>${OS.settings.lang === "ar" ? d.ar : d.name}</b>
                <small>${d.session} · ${d.version} · ${d.pkg}</small>
              </div>
              ${d.id === cur.id ? "✓" : t("switchSession")}
            </button>`).join("")}
        </div>`;
      el.querySelectorAll("[data-id]").forEach((b) => {
        b.onclick = () => {
          applyDistro(b.dataset.id, { persist: true, rerender: true });
          notify(t("session"), currentDistro().name + " · " + currentDistro().session, "distros");
          launch("distros");
        };
      });
    };
